...
> Scope: Sections up to [Flutter Usage](#flutter-usage) are platform-agnostic — they define what each token means and when to use it, and apply to any surface built on this design system. Flutter is the primary implementation target; its bindings are in [Flutter Usage](#flutter-usage).
...
Semantic tokens are the contract. They carry meaning independent of platform, and resolve to concrete values at build time for whatever target consumes them.

**McAfee products are built in Flutter**, so Flutter is the primary implementation target: Material roles resolve to a standard `ColorScheme`, and extended brand/status roles resolve through `AsmTokens`. See [Flutter Usage](#flutter-usage).

Whatever the target, the rule is the same — reason about color by token name and meaning, never by the value behind it.
...
| md.sys.color.surface-variant | md.ref.palette.neutral98 | Alternate surface fill, or inset areas |
...
| md.sys.color.surface-bright | md.ref.palette.neutral100 | Brightest surface; also used for cards and sheets |
...
- `md.sys.color.background` is the dominant app-canvas background, though there are exceptions where `md.sys.color.surface` can also be treated as the app canvas, with `surface-bright` used in combination with it for cards, containers, and sheets.
...
- `md.sys.color.surface-container` is used mainly for tertiary UI elements, especially for components like buttons, text fields, and others.

### Outline & Utility

| Alias Token | Light value (ref) | Usage |
| --- | --- | --- |
| md.sys.color.outline | md.ref.palette.neutral70 | Inputs, forms, and emphasized borders |
| md.sys.color.outline-variant | md.ref.palette.neutral85 | Default borders and dividers |
